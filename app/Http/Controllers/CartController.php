...
class CartController extends Controller
{
    public function getCart(Request $request)
    {
        ...
        $cart = $cart->map(function ($item) {
            ...
            $item->effective_stock = $item->effective_stock;
            return $item;
        });
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($request->variant_id) {
            ...
            if ($variant->stock !== null && $request->quantity > $variant->stock) {
                return $this->error('Requested quantity exceeds available stock', 400);
            }
        } elseif ($product->has_variants) {
            return $this->error('Please select a variant', 400);
        } else {
        ...
        $cart->price = $price;
        $cart->total = $request->quantity * $price;
        $cart->save();
